...
func (c *oauthServerDeploymentSyncer) Sync(ctx context.Context, syncContext factory.SyncContext) (*appsv1.Deployment, bool, bool, string, string, []error) {
	errs := []error{}
	...
	if oidcAvailable, err := common.ExternalOIDCConfigAvailable(c.authLister, c.kasLister, c.kasConfigMapLister); err != nil {
		return nil, false, false, "", "", append(errs, fmt.Errorf("error while checking OIDC config: %v", err))
	} else if oidcAvailable {
		if err := c.deployments.Deployments(expectedDeployment.Namespace).Delete(ctx, expectedDeployment.Name, metav1.DeleteOptions{}); err != nil && !errors.IsNotFound(err) {
			return nil, false, false, "", "", append(errs, fmt.Errorf("error while gracefully deleting deployment: %v", err))
		}
		return nil, false, true, expectedDeployment.Name, expectedDeployment.Namespace, nil
	}
