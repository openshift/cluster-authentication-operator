...
func TestSyncExternalOIDCComponentProxyCA(t *testing.T) {
	destination := resourcesynccontroller.ResourceLocation{Namespace: "openshift-oauth-apiserver", Name: componentProxyCAConfigMapName}
	destinationConfigMap := &corev1.ConfigMap{ObjectMeta: metav1.ObjectMeta{Name: componentProxyCAConfigMapName, Namespace: destination.Namespace}}
	...
	t.Run("trusted CA set and synced mounts the configmap", func(t *testing.T) {
		...
		require.NoError(t, workload.syncComponentProxyCA("proxy-ca", deployment))
		...
		require.Equal(t, componentProxyCAConfigMapName, deployment.Spec.Template.Spec.Volumes[0].Name)
		require.Equal(t, componentProxyCAConfigMapName, deployment.Spec.Template.Spec.Containers[0].VolumeMounts[0].Name)
		require.Equal(t, componentProxyCAMountPath, deployment.Spec.Template.Spec.Containers[0].VolumeMounts[0].MountPath)
	})
	...
	t.Run("trusted CA set before the sync completes returns not found", func(t *testing.T) {
		...
		deployment := oauthAPIServerTestDeployment()
		err := workload.syncComponentProxyCA("proxy-ca", deployment)
	...
	t.Run("trusted CA removed configures destination deletion", func(t *testing.T) {
		...
		require.NoError(t, workload.syncComponentProxyCA("", oauthAPIServerTestDeployment()))
	...
	t.Run("sync errors are returned", func(t *testing.T) {
		...
		require.ErrorContains(t, workload.syncComponentProxyCA("proxy-ca", oauthAPIServerTestDeployment()), "sync failed")
	})
}
