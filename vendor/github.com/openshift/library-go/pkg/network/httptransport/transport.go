...
type transportConfig struct {
	...
	// systemCertPool loads the base trust pool when no WithCertPool is supplied.
	// It defaults to x509.SystemCertPool and exists as an injection seam for tests.
...
func (config *transportConfig) newCertPool() (*x509.CertPool, error) {
	...
	if config.basePool != nil {
		...
		loadSystemCertPool := config.systemCertPool
		if loadSystemCertPool == nil {
			loadSystemCertPool = x509.SystemCertPool
		}

		var err error
		rootCAs, err = loadSystemCertPool()
		if err != nil {
	...
	for _, provider := range config.caContentProviders {
		certificates, err := clientcert.ParseCertsPEM(provider.CurrentCABundleContent())
		if err != nil {
