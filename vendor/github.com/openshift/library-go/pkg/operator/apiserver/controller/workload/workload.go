...
func NewController(name, operatorNamespace, targetNamespace, targetOperandVersion, operandNamePrefix, conditionsPrefix string,
	...
	namespacedInformers []factory.Informer,
	delegate Delegate,
	openshiftClusterConfigClient openshiftconfigclientv1.ClusterOperatorInterface,
	...
	c := factory.New()
	for _, nsi := range namespacedInformers {
...
func (c *Controller) sync(ctx context.Context, controllerContext factory.SyncContext) error {
	...
	if run, err := c.shouldSync(controllerContext, operatorSpec); !run {
		return err
	}
...
func (c *Controller) shouldSync(controllerContext factory.SyncContext, operatorSpec *operatorv1.OperatorSpec) (bool, error) {
	...
	case operatorv1.Removed:
		...
		return false, nil
	default:
		controllerContext.Recorder().Warningf("ManagementStateUnknown", "Unrecognized operator management state %q", operatorSpec.ManagementState)
		return false, nil
	}
}
