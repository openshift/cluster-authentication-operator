...
func (r *runner) run(t testing.TB, workers int, workFunc ...library.DBLoaderFuncType) {
	...
	for i := 0; i < workers; i++ {
		wrapper := func(wg *sync.WaitGroup) {
			...
			if err := runWorkFunctions(kubeClient, "", r.collectError, r.collectStat, workFunc...); err != nil {
				t.Logf("worker finished with loader error: %v", err)
			}
		}
		r.wg.Add(1)
		go wrapper(r.wg)
	...
	if len(r.errorStore) > 0 {
		t.Logf("Workers completed with %d distinct error type(s)", len(r.errorStore))
	} else {
