...
func TestEncryptionRotation(tb testing.TB, scenario library.RotationScenario) {
	tb.Logf("Starting encryption rotation test for %q provider", scenario.EncryptionProvider.Type)
	...
	tb.Logf("Step 2/5: Enabling %s encryption", scenario.EncryptionProvider.Type)
	TestEncryptionType(tb, scenario.BasicScenario, scenario.EncryptionProvider)
	...
	scenario.AssertFunc(tb, libClientSet, scenario.EncryptionProvider.Type, ns, labelSelector)
...
func TestEncryptionProvidersMigration(tb testing.TB, scenario library.ProvidersMigrationScenario) {
	...
	for _, provider := range scenario.EncryptionProviders {
		if provider.Type == configv1.EncryptionTypeIdentity || provider.Type == "" {
			tb.Fatalf("Unsupported encryption provider %q passed", provider.Type)
		}
	}
	...
	for i, provider := range scenario.EncryptionProviders {
		...
		stepName := fmt.Sprintf("%s%s", prefix, string(provider.Type))
		tb.Logf("%s", stepName)
