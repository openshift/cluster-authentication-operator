...
func testPerfEncryptionTypeAESCBC(tt testing.TB) {
	...
	operatorlibrary.TestPerfEncryption(tt, library.PerfScenario{
		...
		AssertDBPopulatedFunc: func(t testing.TB, errorStore map[string]int, statStore map[string]int) {
			require.Empty(t, errorStore, "db loader reported token creation errors")
		...
		EncryptionProvider: library.EncryptionProvider{APIServerEncryption: configv1.APIServerEncryption{Type: configv1.EncryptionTypeAESCBC}},
	})
}
