...
var _ = g.Describe("[sig-auth] authentication operator", func() {
	g.It("[NetworkPolicy][Disruptive] should ensure auth NetworkPolicies are defined", func() {
		testAuthNetworkPolicies()
	})
	g.It("[NetworkPolicy][Disruptive] should restore auth NetworkPolicies after delete or mutation[Timeout:30m]", func() {
