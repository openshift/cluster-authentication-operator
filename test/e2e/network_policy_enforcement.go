...
func testAuthNetworkPolicyEnforcement() {
	...
	allowedServerIPs, cleanupAllowed := createServerPod(ctx, kubeClient, namespace, "np-auth-allowed", serverLabels, 6443)
	defer cleanupAllowed()
	deniedServerIPs, cleanupDenied := createServerPod(ctx, kubeClient, namespace, "np-auth-denied", serverLabels, 12345)
	defer cleanupDenied()
...
func testOAuthAPIServerNetworkPolicyEnforcement() {
	...
	allowedServerIPs, cleanupAllowed := createServerPod(ctx, kubeClient, serverNamespace, "np-oauth-api-allowed", map[string]string{"app": "openshift-oauth-apiserver"}, 8443)
	defer cleanupAllowed()
	deniedServerIPs, cleanupDenied := createServerPod(ctx, kubeClient, serverNamespace, "np-oauth-api-denied", map[string]string{"app": "openshift-oauth-apiserver"}, 12345)
	defer cleanupDenied()
...
func testAuthenticationOperatorNetworkPolicyEnforcement() {
	...
	serverIPs, cleanupServer := createServerPod(ctx, kubeClient, namespace, "np-auth-op-server", serverLabels, 8443)
	defer cleanupServer()
...
func testCrossNamespaceIngressEnforcement() {
	...
	authServerIPs, cleanupAuthServer := createServerPod(ctx, kubeClient, "openshift-authentication", "np-auth-xns", map[string]string{"app": "oauth-openshift"}, 6443)
	defer cleanupAuthServer()
	oauthAPIServerIPs, cleanupOAuthAPIServer := createServerPod(ctx, kubeClient, "openshift-oauth-apiserver", "np-oauth-api-xns", map[string]string{"app": "openshift-oauth-apiserver"}, 8443)
	defer cleanupOAuthAPIServer()
	authOperatorIPs, cleanupAuthOperator := createServerPod(ctx, kubeClient, "openshift-authentication-operator", "np-auth-op-xns", map[string]string{"app": "authentication-operator"}, 8443)
	...
	expectConnectivity(ctx, kubeClient, "openshift-ingress", map[string]string{"test": "arbitrary-client"}, authServerIPs, 6443, true)

	g.By("Testing denied cross-namespace: unauthorized namespace -> oauth-server on unauthorized port")
	expectConnectivity(ctx, kubeClient, "openshift-ingress", map[string]string{"test": "arbitrary-client"}, authServerIPs, 8080, false)
...
func testUnauthorizedNamespaceBlocking() {
	...
	g.By("Creating test server pods in auth namespaces")
	authServerIPs, cleanupAuthServer := createServerPod(ctx, kubeClient, "openshift-authentication", "np-auth-unauth", map[string]string{"app": "oauth-openshift"}, 6443)
	defer cleanupAuthServer()
	oauthAPIServerIPs, cleanupOAuthAPIServer := createServerPod(ctx, kubeClient, "openshift-oauth-apiserver", "np-oauth-api-unauth", map[string]string{"app": "openshift-oauth-apiserver"}, 8443)
	defer cleanupOAuthAPIServer()
	authOperatorIPs, cleanupAuthOperator := createServerPod(ctx, kubeClient, "openshift-authentication-operator", "np-auth-op-unauth", map[string]string{"app": "authentication-operator"}, 8443)
	defer cleanupAuthOperator()
