...
func testGenericNetworkPolicyEnforcement() {
	...
	serverPod := np.NetexecPod(serverName, nsName, serverLabels, 8080)
	_, err = kubeClient.CoreV1().Pods(nsName).Create(context.TODO(), serverPod, metav1.CreateOptions{})
	o.Expect(err).NotTo(o.HaveOccurred())
	o.Expect(np.WaitForPodReady(kubeClient, nsName, serverName)).NotTo(o.HaveOccurred())
	...
	serverIPs := np.PodIPs(server)
	t.Logf("server pod %s/%s ips=%v", nsName, serverName, serverIPs)

	g.By("Verifying allow-all when no policies select the pod")
	np.ExpectConnectivity(t, kubeClient, nsName, clientLabels, serverIPs, 8080, true)

	g.By("Applying default deny and verifying traffic is blocked")
	t.Logf("creating default-deny policy in %s", nsName)
	_, err = kubeClient.NetworkingV1().NetworkPolicies(nsName).Create(context.TODO(), np.DefaultDenyPolicy("default-deny", nsName), metav1.CreateOptions{})
	o.Expect(err).NotTo(o.HaveOccurred())

	g.By("Adding ingress allow only and verifying traffic is still blocked")
	t.Logf("creating allow-ingress policy in %s", nsName)
	_, err = kubeClient.NetworkingV1().NetworkPolicies(nsName).Create(context.TODO(), np.AllowIngressPolicy("allow-ingress", nsName, serverLabels, clientLabels, 8080), metav1.CreateOptions{})
	o.Expect(err).NotTo(o.HaveOccurred())
	np.ExpectConnectivity(t, kubeClient, nsName, clientLabels, serverIPs, 8080, false)

	g.By("Adding egress allow and verifying traffic is permitted")
	t.Logf("creating allow-egress policy in %s", nsName)
	_, err = kubeClient.NetworkingV1().NetworkPolicies(nsName).Create(context.TODO(), np.AllowEgressPolicy("allow-egress", nsName, clientLabels, serverLabels, 8080), metav1.CreateOptions{})
	o.Expect(err).NotTo(o.HaveOccurred())
	np.ExpectConnectivity(t, kubeClient, nsName, clientLabels, serverIPs, 8080, true)
}
...
func testAuthNetworkPolicyEnforcement() {
	...
	allowedServerIPs, cleanupAllowed := np.CreateServerPod(t, kubeClient, namespace, "np-auth-allowed", serverLabels, 6443)
	defer cleanupAllowed()
	deniedServerIPs, cleanupDenied := np.CreateServerPod(t, kubeClient, namespace, "np-auth-denied", serverLabels, 12345)
	defer cleanupDenied()

	g.By("Verifying allowed port 6443")
	np.ExpectConnectivity(t, kubeClient, namespace, clientLabels, allowedServerIPs, 6443, true)
	g.By("Verifying denied port 12345")
	np.ExpectConnectivity(t, kubeClient, namespace, clientLabels, deniedServerIPs, 12345, false)
}
...
func testOAuthAPIServerNetworkPolicyEnforcement() {
	...
	_, err = kubeClient.NetworkingV1().NetworkPolicies(serverNamespace).Get(context.TODO(), "oauth-apiserver-networkpolicy", metav1.GetOptions{})
	o.Expect(err).NotTo(o.HaveOccurred())
	...
	allowedServerIPs, cleanupAllowed := np.CreateServerPod(t, kubeClient, serverNamespace, "np-oauth-api-allowed", map[string]string{"app": "openshift-oauth-apiserver"}, 8443)
	defer cleanupAllowed()
	deniedServerIPs, cleanupDenied := np.CreateServerPod(t, kubeClient, serverNamespace, "np-oauth-api-denied", map[string]string{"app": "openshift-oauth-apiserver"}, 12345)
	defer cleanupDenied()

	g.By("Verifying allowed port 8443")
	np.ExpectConnectivity(t, kubeClient, clientNamespace, clientLabels, allowedServerIPs, 8443, true)

	g.By("Verifying denied port 12345")
	np.ExpectConnectivity(t, kubeClient, clientNamespace, clientLabels, deniedServerIPs, 12345, false)

	g.By("Verifying denied ports even from allowed namespace")
	for _, port := range []int32{80, 443, 6443, 9090} {
		np.ExpectConnectivity(t, kubeClient, clientNamespace, clientLabels, allowedServerIPs, port, false)
	}
	...
	o.Expect(err).NotTo(o.HaveOccurred())
	dnsIPs := np.ServiceClusterIPs(dnsSvc)
	t.Logf("expecting allow from %s to DNS %v:53", serverNamespace, dnsIPs)
	np.ExpectConnectivity(t, kubeClient, serverNamespace, oauthClientLabels, dnsIPs, 53, true)
	...
	o.Expect(err).NotTo(o.HaveOccurred())
	etcdIPs := np.ServiceClusterIPs(etcdSvc)
	...
	np.LogConnectivityBestEffort(t, kubeClient, serverNamespace, oauthClientLabels, etcdIPs, 2379, true)
}
...
func testAuthenticationOperatorNetworkPolicyEnforcement() {
	...
	serverIPs, cleanupServer := np.CreateServerPod(t, kubeClient, namespace, "np-auth-op-server", serverLabels, 8443)
	defer cleanupServer()

	allowedFromSameNamespace := np.IngressAllowsFromNamespace(policy, namespace, clientLabels, 8443)
	g.By("Verifying within-namespace traffic matches policy")
	np.ExpectConnectivity(t, kubeClient, namespace, clientLabels, serverIPs, 8443, allowedFromSameNamespace)

	g.By("Verifying cross-namespace traffic from monitoring is allowed")
	np.ExpectConnectivity(t, kubeClient, "openshift-monitoring", map[string]string{"app.kubernetes.io/name": "prometheus"}, serverIPs, 8443, true)

	g.By("Verifying unauthorized ports are denied")
	np.ExpectConnectivity(t, kubeClient, "openshift-monitoring", map[string]string{"app.kubernetes.io/name": "prometheus"}, serverIPs, 12345, false)
}
...
func testCrossNamespaceIngressEnforcement() {
	...
	authServerIPs, cleanupAuthServer := np.CreateServerPod(t, kubeClient, "openshift-authentication", "np-auth-xns", map[string]string{"app": "oauth-openshift"}, 6443)
	defer cleanupAuthServer()
	oauthAPIServerIPs, cleanupOAuthAPIServer := np.CreateServerPod(t, kubeClient, "openshift-oauth-apiserver", "np-oauth-api-xns", map[string]string{"app": "openshift-oauth-apiserver"}, 8443)
	defer cleanupOAuthAPIServer()
	authOperatorIPs, cleanupAuthOperator := np.CreateServerPod(t, kubeClient, "openshift-authentication-operator", "np-auth-op-xns", map[string]string{"app": "authentication-operator"}, 8443)
	defer cleanupAuthOperator()

	g.By("Testing cross-namespace ingress: auth-operator -> oauth-server:6443")
	np.ExpectConnectivity(t, kubeClient, "openshift-authentication-operator", map[string]string{"app": "authentication-operator"}, authServerIPs, 6443, true)

	g.By("Testing cross-namespace ingress: auth-operator -> oauth-apiserver:8443")
	np.ExpectConnectivity(t, kubeClient, "openshift-authentication-operator", map[string]string{"app": "authentication-operator"}, oauthAPIServerIPs, 8443, true)

	g.By("Testing cross-namespace ingress: oauth-server -> oauth-apiserver:8443")
	np.ExpectConnectivity(t, kubeClient, "openshift-authentication", map[string]string{"app": "oauth-openshift"}, oauthAPIServerIPs, 8443, true)

	g.By("Testing cross-namespace ingress: monitoring -> oauth-server:6443")
	np.ExpectConnectivity(t, kubeClient, "openshift-monitoring", map[string]string{"app.kubernetes.io/name": "prometheus"}, authServerIPs, 6443, true)

	g.By("Testing cross-namespace ingress: monitoring -> oauth-apiserver:8443")
	np.ExpectConnectivity(t, kubeClient, "openshift-monitoring", map[string]string{"app.kubernetes.io/name": "prometheus"}, oauthAPIServerIPs, 8443, true)

	g.By("Testing cross-namespace ingress: monitoring -> auth-operator:8443")
	np.ExpectConnectivity(t, kubeClient, "openshift-monitoring", map[string]string{"app.kubernetes.io/name": "prometheus"}, authOperatorIPs, 8443, true)

	g.By("Testing allow-all ingress: arbitrary namespace -> oauth-server:6443")
	np.ExpectConnectivity(t, kubeClient, "openshift-ingress", map[string]string{"test": "arbitrary-client"}, authServerIPs, 6443, true)

	g.By("Testing denied cross-namespace: unauthorized namespace -> oauth-server on unauthorized port")
	np.ExpectConnectivity(t, kubeClient, "openshift-ingress", map[string]string{"test": "arbitrary-client"}, authServerIPs, 8080, false)

	g.By("Testing allow-all includes other auth components: oauth-apiserver -> oauth-server:6443")
	np.ExpectConnectivity(t, kubeClient, "openshift-oauth-apiserver", map[string]string{"app": "openshift-oauth-apiserver"}, authServerIPs, 6443, true)

	g.By("Testing egress blocking: wrong labels in openshift-authentication (default-deny blocks egress)")
	np.ExpectConnectivity(t, kubeClient, "openshift-authentication", map[string]string{"app": "wrong-app"}, oauthAPIServerIPs, 8443, false)
}
...
func testUnauthorizedNamespaceBlocking() {
	...
	g.By("Creating test server pods in auth namespaces")
	authServerIPs, cleanupAuthServer := np.CreateServerPod(t, kubeClient, "openshift-authentication", "np-auth-unauth", map[string]string{"app": "oauth-openshift"}, 6443)
	defer cleanupAuthServer()
	oauthAPIServerIPs, cleanupOAuthAPIServer := np.CreateServerPod(t, kubeClient, "openshift-oauth-apiserver", "np-oauth-api-unauth", map[string]string{"app": "openshift-oauth-apiserver"}, 8443)
	defer cleanupOAuthAPIServer()
	authOperatorIPs, cleanupAuthOperator := np.CreateServerPod(t, kubeClient, "openshift-authentication-operator", "np-auth-op-unauth", map[string]string{"app": "authentication-operator"}, 8443)
	defer cleanupAuthOperator()
	...
	np.ExpectConnectivity(t, kubeClient, "default", map[string]string{"test": "any-pod"}, authServerIPs, 6443, true)

	g.By("Testing allow-all rules: oauth-apiserver:8443 (kube-apiserver webhook/aggregated APIs)")
	np.ExpectConnectivity(t, kubeClient, "default", map[string]string{"test": "any-pod"}, oauthAPIServerIPs, 8443, true)

	g.By("Testing strict blocking: unauthorized namespace -> auth-operator:8443")
	defaultAllowed := np.IngressAllowsFromNamespace(authOperatorPolicy, "default", map[string]string{"test": "unauthorized"}, 8443)
	np.ExpectConnectivity(t, kubeClient, "default", map[string]string{"test": "unauthorized"}, authOperatorIPs, 8443, defaultAllowed)
	...
	np.ExpectConnectivity(t, kubeClient, "openshift-etcd", map[string]string{"test": "unauthorized"}, authOperatorIPs, 8443, false)

	g.By("Testing port-based blocking: unauthorized port even from any namespace")
	np.ExpectConnectivity(t, kubeClient, "default", map[string]string{"test": "any-pod"}, oauthAPIServerIPs, 9999, false)

	g.By("Testing port-based blocking: unauthorized port on oauth-server")
	np.ExpectConnectivity(t, kubeClient, "default", map[string]string{"test": "any-pod"}, authServerIPs, 9999, false)

	g.By("Testing allow-all ingress: wrong labels from allowed namespace")
	np.ExpectConnectivity(t, kubeClient, "openshift-monitoring", map[string]string{"app": "wrong-label"}, authOperatorIPs, 8443, true)

	g.By("Testing egress blocking: wrong labels in openshift-authentication (default-deny blocks egress)")
	np.ExpectConnectivity(t, kubeClient, "openshift-authentication", map[string]string{"app": "wrong-label"}, oauthAPIServerIPs, 8443, false)

	g.By("Testing multiple unauthorized ports on oauth-server")
	for _, port := range []int32{80, 443, 8080, 8443, 22, 3306} {
		np.ExpectConnectivity(t, kubeClient, "default", map[string]string{"test": "any-pod"}, authServerIPs, port, false)
	}

	g.By("Testing allow-all ingress: oauth-server can reach auth-operator")
	np.ExpectConnectivity(t, kubeClient, "openshift-authentication", map[string]string{"app": "oauth-openshift"}, authOperatorIPs, 8443, true)
}
