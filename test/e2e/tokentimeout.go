...
var _ = g.Describe("[sig-auth] authentication operator", func() {
	g.It("[Parallel][Tokens] TestTokenInactivityTimeout [Timeout:1h]", func() {
...
func testTokenInactivityTimeout(t testing.TB) {
	...
	testTokenTimeouts := func(t testing.TB, tokenWithTimeout, tokenWithoutTimeout string, timeout time.Duration) {
	...
	testInactivityTimeoutScenarios := func(t testing.TB, tokenWithTimeout, tokenWithoutTimeout string, timeout time.Duration) {
		req, err := http.NewRequest(http.MethodGet, kubeConfig.Host+"/apis/user.openshift.io/v1/users/~", &bytes.Buffer{})
		require.NoError(t, err)
