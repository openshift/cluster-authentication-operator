...
const testID = "cao-oidc-e2e-configure"
...
func ConfigureSuiteBeforeAll() {
	...
	err = externaloidc.DeployKeycloak(ctx, oc, keycloakNamespace, g.GinkgoLogr)
	o.Expect(err).NotTo(o.HaveOccurred(), "should not encounter an error deploying keycloak")

	kcURL, err := admittedURLForRoute(ctx, oc, externaloidc.KeycloakResourceName, keycloakNamespace)
	...
	err = keycloakCli.Authenticate("admin-cli", externaloidc.KeycloakAdminUsername, externaloidc.KeycloakAdminPassword)
...
var _ = g.Describe("[sig-auth][Suite:cluster-authentication-operator/externaloidc/configure][Serial][Slow][Disruptive]", g.Ordered, func() {
